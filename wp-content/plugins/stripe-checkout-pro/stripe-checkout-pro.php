 ...
 * Description: Add highly optimized Stripe checkout form overlays to your site in a few simple steps. **Pro Version**
...
$stripe_checkout_constants = array(
	...
	'SC_DIR_PATH_PRO'     => plugin_dir_path( __FILE__ ),
...
include_once 'stripe-checkout-pro-requirements.php';
$stripe_checkout_requirements = new Stripe_Checkout_Pro_Requirements( $stripe_checkout_requires );
	...
	if ( isset( $stripe_checkout_fails['wp'] ) || isset( $stripe_checkout_fails['php']) ) {
		...
		function stripe_checkout_plugin_requirements() {
			...
			echo '<div class="error">' .
					'<p>'  .
			            sprintf(
				            __( 'WP Simple Pay Pro requires PHP %1$s and WordPress %2$s to function properly. PHP version found: %3$s. WordPress installed version: %4$s. Please upgrade to meet the minimum requirements. <a href="http://www.wpupdatephp.com/update/" target=_blank">Read more on why it is important to stay updated.</a>', 'sc' ),
				            $required['php'],
				            $required['wp'],
				            PHP_VERSION,
				            $wp_version
